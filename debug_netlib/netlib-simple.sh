...
#
# Changelog v1.1:
#   [C-1] pf anchor: added 'set block-policy drop' + 'set skip on lo0' (NIST 800-41)
#   [C-2] pf anchor: added 'scrub in/out' for packet normalization (NIST 800-41)
#   [C-3] pf anchor: 'block drop log' for forensic trail (NIST 800-92)
#         _install_pf_launchdaemon: initialize pflog0 + blocked log file
#   [H-1] ask(): replaced eval with printf -v (CWE-78)
#   [H-2] update_hosts(): content sanity check + min size guard (NIST SSDF PW.4.1)
#   [H-3] _install_pf_launchdaemon: explicit log file permissions 640 (CIS macOS)
#   [H-4] pf anchor: ICMPv6 comment updated per RFC 4890 §4.4 desktop exception
#   [M-1] curl: added --retry 3 --max-time 30 --tlsv1.2 --proto '=https'
#   [M-2] disable_pf_dns_lock: replaced fragile awk with grep -n + head -n
#
# Requirements:
...
# [H-1] FIX: replaced eval with printf -v (CWE-78 — no command injection)
ask() {
...
# Generated by netlib-simple.sh v1.1
# References: NIST SP 800-41 Rev1, RFC 4890, OpenBSD pf docs
# ============================================================

# [C-1] Default-deny policy for this anchor (NIST SP 800-41 §4.3)
# Ensures no traffic passes unless explicitly permitted below,
# regardless of global /etc/pf.conf policy.
set block-policy drop

# [C-1] Loopback exempt — mDNSResponder binds here.
# 'set skip' is cleaner than 'pass quick on lo0 all':
# skip removes lo0 from pf processing entirely.
set skip on lo0

# [C-2] Packet normalization (NIST SP 800-41 §4.2.3, OpenBSD pf FAQ §9)
# - fragment reassemble: prevents fragmentation overlap attacks (CVE-2008-1447)
# - random-id: randomizes IP ID field, prevents OS fingerprinting
scrub in  all fragment reassemble
scrub out all random-id

# --- ICMPv6: RFC 4890 §4.3.1 requires types 1-4 (errors), 128-129 (echo),
# 133-137 (NDP), 143 (MLD) for IPv6 connectivity.
# 'pass all' is accepted for desktop/personal use per RFC 4890 §4.4.
# Enterprise deployments should restrict to specific ICMPv6 types.
pass quick inet6 proto icmp6 all

# --- Tailscale DNS: MagicDNS resolver (100.64.0.0/10 CGNAT range)
# External to lo0 — must be explicitly permitted.
pass out quick proto { udp tcp } to 100.100.100.100 port 53

# --- Quad9 DoH: port 443 ONLY
# 9.9.9.9  = standard Quad9
# 9.9.9.11 = ECS-compatible (used by macOS DoH profile — must be listed)
# 149.112.112.112 = Quad9 secondary
# Port restriction: prevents plain DNS (port 53) leaks to Quad9 IPs.
# Without port restriction, 'pass to 9.9.9.9' would allow 9.9.9.9:53
# before 'block port 53' fires (pf 'quick' = first match wins).
...
# --- Block plain DNS (IPv4) — NIST SP 800-41 §4.3.1
# 'block drop log' = silent drop + forensic log to pflog0 (NIST SP 800-92 §3.3.3)
# 'block drop'     = silent drop, no ICMP unreachable (hides firewall presence)
# 'block out'      = sends ICMP unreachable (reveals firewall to attacker)
...
_install_pf_launchdaemon() {
    ...
    local BLOCKED_LOG="/var/log/foxhole-pf-blocked.log"
    local PF_LOG="/var/log/foxhole-pf.log"

    # [H-3] Explicit log file creation with restricted permissions (CIS macOS §6.1)
    # 640 = root writes, wheel reads, others: no access
    sudo touch "$BLOCKED_LOG" "$PF_LOG"
    sudo chown root:wheel "$BLOCKED_LOG" "$PF_LOG"
    sudo chmod 640 "$BLOCKED_LOG" "$PF_LOG"
    log "  [✓] Log files initialized with 640 permissions"
...
<dict>
    ...
    <array>
        ...
        <string>-c</string>
        <string>
            /sbin/pfctl -e -f /etc/pf.conf 2>/dev/null;
            /sbin/pfctl -a com.hardening.dnsleak -f /etc/pf.anchors/com.hardening.dnsleak;
            /sbin/ifconfig pflog0 create 2>/dev/null || true;
            /usr/sbin/tcpdump -n -e -l -i pflog0 >> ${BLOCKED_LOG} 2>&1 &
        </string>
    </array>
    <key>RunAtLoad</key>
    <true/>
    <key>StandardErrorPath</key>
    <string>${PF_LOG}</string>
</dict>
</plist>
EOF
    ...
    log "  [✓] pf LaunchDaemon installed (pflog0 + auto-loads on boot)"
    info "  Blocked DNS attempts logged to: ${BLOCKED_LOG}"
}

# [M-2] FIX: replaced fragile awk-regex with grep -n + head -n (portable)
disable_pf_dns_lock() {
    local PF_CONF="/etc/pf.conf"
    local PF_MARKER="# ===== hardening dns lock ====="
...
update_hosts() {
    ...
    # [M-1] FIX: added --tlsv1.2 --proto '=https' --retry --max-time (NIST 800-52)
    if ! curl -fsSL --retry 3 --max-time 30 \
              --tlsv1.2 --proto '=https' \
              "$HOSTS_URL" -o "$TMP_HOSTS"; then
        rm -f "$TMP_HOSTS"
        die "Failed to download StevenBlack hosts!"
    fi

    # [H-2] FIX: content sanity checks before writing to /etc/hosts (NIST SSDF PW.4.1)
    # Check 1: verify canonical StevenBlack header is present
    if ! grep -q "^# Title: StevenBlack" "$TMP_HOSTS"; then
        rm -f "$TMP_HOSTS"
        die "Content validation failed — not a StevenBlack hosts file. Possible MITM. Aborting."
    fi
    # Check 2: minimum file size guard (authentic file is ~2MB; 500KB = safe lower bound)
    local SIZE
    SIZE=$(wc -c < "$TMP_HOSTS")
    ...
    local LATEST_COMMIT
    LATEST_COMMIT=$(curl -fsSL --max-time 10 --tlsv1.2 --proto '=https' \
